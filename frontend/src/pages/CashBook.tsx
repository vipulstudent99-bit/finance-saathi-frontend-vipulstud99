...
import { Banknote, Calendar, Download } from 'lucide-react';
...
async function getCashBook(from?: string, to?: string) {
  const p = new URLSearchParams();
  if (from) p.set('from', from);
  if (to) p.set('to', to);
  const qs = p.toString();
  const res = await fetch(`${BASE}/reports/cash-book${qs ? `?${qs}` : ''}`);
...
export default function CashBook() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(e.message ?? 'Error loading cash book');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-8 space-y-6" data-testid="cash-book-page">
      {/* Header */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">All cash inflows and outflows</p>
          </div>
        </div>
      </div>
      ...
      {!loading && data && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-4">
            ...
            <p className="text-xs text-slate-400">Cash paid</p>
          </div>
          <div className="bg-white rounded-xl border border-emerald-200 bg-emerald-50 p-4">
            <p className="text-xs text-emerald-700 uppercase tracking-wider">Closing Balance</p>
            <p className="text-xl font-mono font-bold text-emerald-700 mt-1">{formatCurrency(data.closingBalance)}</p>
            <p className="text-xs text-emerald-600">{data.closingBalanceSide}</p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm" data-testid="cash-book-table">
            <thead>
              ...
              {loading ? (
                ...
                data.transactions.map((tx, i) => (
                  <tr key={tx.voucherId + i} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-right font-mono">
                      <span className={tx.balanceSide === 'DR' ? 'text-emerald-700 font-bold' : 'text-rose-700 font-bold'}>
